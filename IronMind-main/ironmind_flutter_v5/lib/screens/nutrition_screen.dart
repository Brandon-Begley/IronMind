  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(controller: _tabs, children: [
        _TodayTab(date: _today, targets: _targets, onAddFood: _showAddFood),
        _NewPlanTab(targets: _targets),
        const _MyPlansTab(),
      ]),
    );
  }
...
class _TodayTab extends StatefulWidget {
  ...
  const _TodayTab({required this.date, required this.targets, required this.onAddFood});
  @override
...
class _NewPlanTab extends StatefulWidget {
  final Map<String, dynamic> targets;
  const _NewPlanTab({required this.targets});
  @override
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        IronButton(label: 'GENERATE MEAL PLAN', loading: _loading, onPressed: () async {
          ...
          try {
            await ApiService.generateMealPlan(goal: _goals[_goal], days: int.parse(_duration), preferences: _prefsCtrl.text);
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (_plan == null)
          ...
          IronCard2(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            IronGhostButton(label: 'VIEW FULL PLAN', onPressed: () {}, color: IronMindTheme.text2),
          ])),
        const SizedBox(height: 12),
        IronButton(label: '+ GENERATE NEW PLAN', onPressed: () {}),
      ]),
    );
  }
}
